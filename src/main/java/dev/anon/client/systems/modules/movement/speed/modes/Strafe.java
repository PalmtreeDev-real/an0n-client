    ...
    public void onMove(PlayerMoveEvent event) {
        switch (stage) {
            ...
            case 1 -> { // Jump
                ...
                ((IVec3) event.movement).anon$setY(getHop(0.40123128));
        ...
        ((IVec3) event.movement).anon$setXZ(change.x, change.y);
    }

    public static Vector2d transformStrafe(double speed) {
        float forward = Math.signum(AnonClient.mc.player.input.getMoveVector().y);
        float side = Math.signum(AnonClient.mc.player.input.getMoveVector().x);
        float yaw = AnonClient.mc.player.getYRot(AnonClient.mc.getDeltaTracker().getGameTimeDeltaPartialTick(true));
