...
import dev.anon.client.mixin.ItemStackRenderStateAccessor;
import dev.anon.client.mixin.LayerRenderStateAccessor;
...
public class ItemPhysics extends Module {
    ...
    private void onRenderItemEntity(RenderItemEntityEvent event) {
        ...
        for (int i = 0; i < ((ItemStackRenderStateAccessor) event.renderState.item).anon$getActiveLayerCount(); i++) {
            ItemStackRenderState.LayerRenderState layer = ((ItemStackRenderStateAccessor) event.renderState.item).anon$getLayers()[i];
            ModelInfo info = getInfo(layer.prepareQuadList());

            matrices.pushPose();
            applyTransformation(matrices, ((LayerRenderStateAccessor) layer).anon$getTransform());
