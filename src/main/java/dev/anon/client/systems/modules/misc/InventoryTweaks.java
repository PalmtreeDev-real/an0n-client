...
import dev.anon.client.events.entity.player.InteractEntityEvent;
...
import dev.anon.client.mixin.AbstractContainerScreenAccessor;
...
public class InventoryTweaks extends Module {
    ...
    private boolean sort() {
        ...
        Slot focusedSlot = ((AbstractContainerScreenAccessor) screen).anon$getHoveredSlot();
        ...
        for (int i = start; i < end; i++) {
            ...
            if (sleep > 0) {
                try {
                    ...
                    AnonClient.LOG.error("Error when sleeping the slot mover", e);
                }
            }
    ...
    public void steal(AbstractContainerMenu handler) {
        AnonExecutor.execute(() -> moveSlots(handler, 0, SlotUtils.indexToId(SlotUtils.MAIN_START), true));
    }

    public void dump(AbstractContainerMenu handler) {
        int playerInvOffset = SlotUtils.indexToId(SlotUtils.MAIN_START);
        AnonExecutor.execute(() -> moveSlots(handler, playerInvOffset, playerInvOffset + 4 * 9, false));
    }
