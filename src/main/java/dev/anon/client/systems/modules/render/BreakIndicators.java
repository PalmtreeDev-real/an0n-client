...
import dev.anon.client.mixin.LevelRendererAccessor;
...
public class BreakIndicators extends Module {
    ...
    private void renderNormal(Render3DEvent event) {
        Map<Integer, BlockDestructionProgress> blocks = ((LevelRendererAccessor) mc.levelRenderer).anon$getDestroyingBlocks();

        float ownBreakingStage = ((MultiPlayerGameModeAccessor) mc.gameMode).anon$getBreakingProgress();
        BlockPos ownBreakingPos = ((MultiPlayerGameModeAccessor) mc.gameMode).anon$getCurrentBreakingBlockPos();
