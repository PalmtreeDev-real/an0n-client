...
import dev.anon.client.mixin.DirectionAccessor;
...
public class AutoWasp extends Module {
    ...
    private void onMove(PlayerMoveEvent event) {
        ...
        if (avoidLanding.get()) {
            ...
            for (Direction dir : DirectionAccessor.anon$getHorizontal()) {
                BlockPos pos = BlockPos.containing(targetPos.relative(dir, d).relative(dir.getClockWise(), d)).below();
                if (((BlockBehaviourAccessor) mc.level.getBlockState(pos).getBlock()).anon$isHasCollision() && Math.abs(targetPos.y() - (pos.getY() + 1)) <= 0.25) {
        ...
        ((IVec3) event.movement).anon$set(xVel, yVel, zVel);
    }
