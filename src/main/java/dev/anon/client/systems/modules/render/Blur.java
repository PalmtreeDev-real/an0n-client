...
import dev.anon.client.renderer.FixedUniformStorage;
...
public class Blur extends Module {
    ...
    private final Setting<Boolean> anon = sgScreens.add(new BoolSetting.Builder()
        .name("anon-screens")
        .description("Applies blur to AN0N screens.")
        .defaultValue(true)
        .build());
    ...
    public Blur() {
        ...
        AnonClient.EVENT_BUS.subscribe(new ConsumerListener<>(ResolutionChangedEvent.class, event -> {
            // Resize all fbos
            for (int i = 0; i < fbos.length; i++) {
                if (fbos[i] != null) {
                    fbos[i].close();
        ...
        AnonClient.EVENT_BUS.subscribe(new ConsumerListener<>(RenderAfterWorldEvent.class, event -> onRenderAfterWorld()));
    }
    ...
    private void onRenderAfterWorld() {
        ...
        // Initial downsample
        renderToFbo(fbos[0], mc.getMainRenderTarget().getColorTextureView(), AnonRenderPipelines.BLUR_DOWN, ubos[0]);

        // Downsample
        for (int i = 0; i < iterations; i++) {
            renderToFbo(fbos[i + 1], fbos[i], AnonRenderPipelines.BLUR_DOWN, ubos[i + 1]);
        }

        // Upsample
        for (int i = iterations; i >= 1; i--) {
            renderToFbo(fbos[i - 1], fbos[i], AnonRenderPipelines.BLUR_UP, ubos[i - 1]);
        }

        // Render output
        MeshRenderer.begin()
            .attachments(mc.getMainRenderTarget())
            .pipeline(AnonRenderPipelines.BLUR_PASSTHROUGH)
            .fullscreen()
    ...
    private boolean shouldRender() {
        ...
        if (screen instanceof WidgetScreen) return anon.get();
    ...
    private static final FixedUniformStorage<BlurUniformData> UNIFORM_STORAGE = new FixedUniformStorage<>("AN0N - Blur UBO", UNIFORM_SIZE, 6);
