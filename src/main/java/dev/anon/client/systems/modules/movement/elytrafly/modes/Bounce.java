...
package dev.anon.client.systems.modules.movement.elytrafly.modes;

import dev.anon.client.AnonClient;
import dev.anon.client.events.packets.PacketEvent;
import dev.anon.client.systems.modules.movement.elytrafly.ElytraFlightMode;
    ...
    private static boolean startGliding(LocalPlayer player) {
        for (EquipmentSlot equipmentSlot : EquipmentSlot.VALUES) {
            if (LivingEntity.canGlideUsing(player.getItemBySlot(equipmentSlot), equipmentSlot)) {
                AnonClient.mc.executeIfPossible(player::startFallFlying);
                return true;
            }
        }
