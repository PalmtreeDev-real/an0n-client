...
import dev.anon.client.mixin.FileCacheAccessor;
import dev.anon.client.mixin.MinecraftAccessor;
import dev.anon.client.mixin.SkinManagerAccessor;
...
public abstract class Account<T extends Account<?>> implements ISerializable<T> {
    ...
    public static void setSession(User session) {
        ...
        mca.anon$setUserApiService(apiService);
        mca.anon$setPlayerSocialManager(new PlayerSocialManager(mc, apiService));
        mca.anon$setProfileKeyPairManager(ProfileKeyPairManager.create(apiService, session, mc.gameDirectory.toPath()));
        mca.anon$setReportingContext(ReportingContext.create(ReportEnvironment.local(), apiService));
        mca.anon$setProfileFuture(CompletableFuture.supplyAsync(() -> mc.services().sessionService().fetchProfile(mc.getUser().getProfileId(), true), Util.ioPool()));
    }
    ...
    public static void applyLoginEnvironment(YggdrasilAuthenticationService authService) {
        ...
        SkinManager.TextureCache skinCache = ((SkinManagerAccessor) mc.getSkinManager()).anon$getSkinTextures();
        Path skinCachePath = ((FileCacheAccessor) skinCache).anon$getRoot();
        mca.anon$setServices(Services.create(authService, mc.gameDirectory));
        mca.anon$setSkinManager(new SkinManager(skinCachePath, mc.services(), new SkinTextureDownloader(mc.getProxy(), mc.getTextureManager(), mc), mc));
    }
