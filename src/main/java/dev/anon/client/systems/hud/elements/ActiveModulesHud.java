...
import dev.anon.client.systems.hud.*;
...
public class ActiveModulesHud extends HudElement {
    ...
    private void renderModule(HudRenderer renderer, int index, double x, double y) {
        ...
        if (module.hasGradient()) {
            Color[] grad = module.getGradientColors();
            String title = module.title;
            textHeight = renderer.textHeight(shadow.get(), getScale());
            textLength = 0;
            double charX = x;
            for (int i = 0; i < title.length(); i++) {
                float t = title.length() > 1 ? (float) i / (title.length() - 1) : 0;
                Color c = new Color(
                    lerp(grad[0].r, grad[1].r, t),
                    lerp(grad[0].g, grad[1].g, t),
                    lerp(grad[0].b, grad[1].b, t)
                );
                String ch = String.valueOf(title.charAt(i));
                renderer.text(ch, charX, y, c, shadow.get(), getScale());
                double w = renderer.textWidth(ch, shadow.get(), getScale());
                charX += w;
                textLength += w;
            }
        } else {
            ...
            textHeight = renderer.textHeight(shadow.get(), getScale());
            textLength = renderer.textWidth(module.title, shadow.get(), getScale());
        }
