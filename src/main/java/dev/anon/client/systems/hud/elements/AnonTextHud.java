...
public class AnonTextHud {
    public static final HudElementInfo<TextHud> INFO = new HudElementInfo<>(Hud.GROUP, "text", "Displays arbitrary text with Starscript.", AnonTextHud::create);
    ...
    static {
        ...
        MODULE_ENABLED = addPreset("Module enabled", "Kill Aura: {anon.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"}", 0);
        MODULE_ENABLED_WITH_INFO = addPreset("Module enabled with info", "Kill Aura: {anon.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"} #1{anon.get_module_info(\"kill-aura\")}", 0);
        WATERMARK = addPreset("Watermark", "AN0N CLIENT");
