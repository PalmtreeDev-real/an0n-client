...
import dev.anon.client.gui.DefaultSettingsWidgetFactory;
...
import dev.anon.client.gui.themes.anon.widgets.*;
import dev.anon.client.gui.themes.anon.widgets.input.WAnonDropdown;
import dev.anon.client.gui.themes.anon.widgets.input.WAnonSlider;
import dev.anon.client.gui.themes.anon.widgets.input.WAnonTextBox;
import dev.anon.client.gui.themes.anon.widgets.pressable.*;
import dev.anon.client.gui.utils.AlignmentX;
import dev.anon.client.gui.utils.CharFilter;
import dev.anon.client.gui.widgets.*;
import dev.anon.client.gui.widgets.containers.WSection;
import dev.anon.client.gui.widgets.containers.WView;
import dev.anon.client.gui.widgets.containers.WWindow;
...
import dev.anon.client.gui.widgets.pressable.*;
...
public class AnonGuiTheme extends GuiTheme {
    ...
    public final Setting<SettingColor> accentColor = color("accent", "Main color of the GUI.", new SettingColor(50, 50, 50));
    public final Setting<SettingColor> checkboxColor = color("checkbox", "Color of checkbox.", new SettingColor(180, 180, 180));
    public final Setting<SettingColor> plusColor = color("plus", "Color of plus button.", new SettingColor(180, 180, 180));
    public final Setting<SettingColor> minusColor = color("minus", "Color of minus button.", new SettingColor(180, 180, 180));
    ...
    public final ThreeStateColorSetting sliderHandle = new ThreeStateColorSetting(
        sgSlider,
        "slider-handle",
        new SettingColor(80, 80, 80),
        new SettingColor(100, 100, 100),
        new SettingColor(120, 120, 120)
    );

    public final Setting<SettingColor> sliderLeft = color(sgSlider, "slider-left", "Color of slider left part.", new SettingColor(70, 70, 70));
    public final Setting<SettingColor> sliderRight = color(sgSlider, "slider-right", "Color of slider right part.", new SettingColor(30, 30, 30));
    ...
    public WWindow window(WWidget icon, String title) {
        return w(new WAnonWindow(icon, title));
    }

    @Override
    public WLabel label(String text, boolean title, double maxWidth) {
        if (maxWidth == 0 && !text.contains("\n")) return w(new WAnonLabel(text, title));
        return w(new WAnonMultiLabel(text, title, maxWidth));
    }

    @Override
    public WHorizontalSeparator horizontalSeparator(String text) {
        return w(new WAnonHorizontalSeparator(text));
    }

    @Override
    public WVerticalSeparator verticalSeparator() {
        return w(new WAnonVerticalSeparator());
    }

    @Override
    protected WButton button(String text, GuiTexture texture) {
        return w(new WAnonButton(text, texture));
    }

    @Override
    protected WConfirmedButton confirmedButton(String text, String confirmText, GuiTexture texture) {
        return w(new WAnonConfirmedButton(text, confirmText, texture));
    }

    @Override
    public WMinus minus() {
        return w(new WAnonMinus());
    }

    @Override
    public WConfirmedMinus confirmedMinus() {
        return w(new WAnonConfirmedMinus());
    }

    @Override
    public WPlus plus() {
        return w(new WAnonPlus());
    }

    @Override
    public WCheckbox checkbox(boolean checked) {
        return w(new WAnonCheckbox(checked));
    }

    @Override
    public WSlider slider(double value, double min, double max) {
        return w(new WAnonSlider(value, min, max));
    }

    @Override
    public WTextBox textBox(String text, String placeholder, CharFilter filter, Class<? extends WTextBox.Renderer> renderer) {
        return w(new WAnonTextBox(text, placeholder, filter, renderer));
    }

    @Override
    public <T> WDropdown<T> dropdown(T[] values, T value) {
        return w(new WAnonDropdown<>(values, value));
    }

    @Override
    public WTriangle triangle() {
        return w(new WAnonTriangle());
    }

    @Override
    public WTooltip tooltip(String text) {
        return w(new WAnonTooltip(text));
    }

    @Override
    public WView view() {
        return w(new WAnonView());
    }

    @Override
    public WSection section(String title, boolean expanded, WWidget headerWidget) {
        return w(new WAnonSection(title, expanded, headerWidget));
    }

    @Override
    public WAccount account(WidgetScreen screen, Account<?> account) {
        return w(new WAnonAccount(screen, account));
    }

    @Override
    public WWidget module(Module module, String title) {
        return w(new WAnonModule(module, title));
    }

    @Override
    public WQuad quad(Color color) {
        return w(new WAnonQuad(color));
    }

    @Override
    public WTopBar topBar() {
        return w(new WAnonTopBar());
    }

    @Override
    public WFavorite favorite(boolean checked) {
        return w(new WAnonFavorite(checked));
    }
