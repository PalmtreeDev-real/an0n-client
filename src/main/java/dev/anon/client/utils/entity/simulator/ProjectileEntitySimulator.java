...
import dev.anon.client.mixin.CrossbowItemAccessor;
import dev.anon.client.mixin.ProjectileInGroundAccessor;
...
public class ProjectileEntitySimulator {
    ...
    public boolean set(Entity user, ItemStack itemStack, double angleOffset, boolean accurate, float tickDelta) {
        ...
        switch (item) {
            ...
            case CrossbowItem _ -> {
                ...
                float speed = CrossbowItemAccessor.anon$getSpeed(projectilesComponent);
    ...
    public boolean set(Entity entity) {
        ...
        if (entity instanceof ProjectileInGroundAccessor ppe && ppe.anon$invokeIsInGround()) return false;
    ...
    private SimulationStep getCollision() {
        ...
        if (blockCollision.getType() != HitResult.Type.MISS) {
            ((IVec3) pos3d).anon$set(blockCollision.getLocation());
        }
