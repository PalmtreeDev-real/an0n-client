...
public class Capes {
    private static final String CAPE_OWNERS_URL = "https://anonclient.com/api/capeowners";
    private static final String CAPES_URL = "https://anonclient.com/api/capes";
    ...
    @PreInit(dependencies = AnonExecutor.class)
    public static void init() {
        ...
        AnonExecutor.execute(() -> {
            ...
            Stream<String> lines = Http.get(CAPE_OWNERS_URL)
                .exceptionHandler(e -> AnonClient.LOG.error("Could not load capes: {}", e.getMessage()))
                .sendLines();
            if (lines != null) {
                lines.forEach(s -> {
        ...
        AnonClient.EVENT_BUS.subscribe(Capes.class);
    }
    ...
    private static class Cape {
        ...
        public Cape(String name) {
            this.identifier = AnonClient.identifier("capes/" + COUNT++);
            this.name = name;
        }
        ...
        public void download() {
            ...
            AnonExecutor.execute(() -> {
                try {
                    ...
                    AnonClient.LOG.error("Failed to download cape '{}'", name, e);
                }
            });
        }
