...
public class ChamsShader extends EntityShader {
    ...
    public ChamsShader() {
        super(AnonRenderPipelines.POST_IMAGE);
        AnonClient.EVENT_BUS.subscribe(ChamsShader.class);
    }
    ...
    public static void load() {
        try {
            ...
            for (String fileFormat : FILE_FORMATS) {
                Optional<Resource> optional = mc.getResourceManager().getResource(AnonClient.identifier("textures/chams." + fileFormat));
            ...
            AnonClient.LOG.error("Error loading the chams shader", e);
        }
    }
    ...
    private static final DynamicUniformStorage<UniformData> UNIFORM_STORAGE = new DynamicUniformStorage<>("AN0N - Image UBO", UNIFORM_SIZE, 16);
