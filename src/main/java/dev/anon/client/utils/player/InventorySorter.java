...
public class InventorySorter {
    ...
    private void generateActions() {
        ...
        for (Slot slot : screen.getMenu().slots) {
            if (getInvPart(slot) == originInvPart)
                slots.add(new MySlot(((ISlot) slot).anon$getIndex(), slot.getItem()));
        }
    ...
    private InvPart getInvPart(Slot slot) {
        int i = ((ISlot) slot).anon$getSlot();

        if (slot.container instanceof Inventory && (!(screen instanceof CreativeModeInventoryScreen) || ((ISlot) slot).anon$getIndex() > 8)) {
