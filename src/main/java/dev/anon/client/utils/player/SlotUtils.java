...
import dev.anon.client.mixin.AbstractMountInventoryMenuAccessor;
import dev.anon.client.mixin.CreativeModeInventoryScreenAccessor;
import dev.anon.client.mixin.CreativeModeTabsAccessor;
...
public class SlotUtils {
    ...
    private static int creativeInventory(int i) {
        if (CreativeModeInventoryScreenAccessor.anon$getSelectedTab() != BuiltInRegistries.CREATIVE_MODE_TAB.getValue(CreativeModeTabsAccessor.anon$getInventory()))
            return -1;
    ...
    private static int horse(AbstractContainerMenu handler, int i) {
        LivingEntity entity = ((AbstractMountInventoryMenuAccessor) handler).anon$getMount();
