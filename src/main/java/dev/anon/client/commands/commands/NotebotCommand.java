...
import dev.anon.client.commands.arguments.NotebotSongArgumentType;
...
public class NotebotCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<ClientSuggestionProvider> builder) {
        builder.then(literal("help").executes(_ -> {
            Util.getPlatform().openUri("https://github.com/Palmtreedev-real/AN0N/wiki");
        ...
        builder.then(literal("record").then(literal("save").then(argument("name", StringArgumentType.greedyString()).executes(ctx -> {
            ...
            Path notebotFolder = AnonClient.FOLDER.toPath().resolve("notebot");
