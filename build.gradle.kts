...
loom {
    accessWidenerPath = file("src/main/resources/anon-client.classtweaker")
}

fun toMinecraftCompat(version: String): String {
    val match = Regex("""^(\d+)\.(\d+)(?:\.(\d+))?$""")
        .matchEntire(version)
        ?: error("Invalid Minecraft version format: $version. Expected YY.D.H or 1.XX.XX")
...
tasks {
    ...
    jar {
        ...
        manifest {
            attributes["Main-Class"] = "dev.anon.client.Main"
        }
    }
...
publishing {
    ...
    repositories {
        maven("https://maven.meteordev.org/snapshots") {
            ...
            credentials {
                username = System.getenv("MAVEN_ANON_ALIAS")
                password = System.getenv("MAVEN_ANON_TOKEN")
            }
